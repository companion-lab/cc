...
/// Top-level configuration for c2.
///
/// Loaded in priority order (highest wins):
///   1. System-managed  (/etc/c2/config.json or %ProgramData%\c2\config.json)
///   2. User global     (~/.config/c2/config.json)
///   3. Project local   (.c2/config.json in working directory)
...
pub async fn load(working_dir: &PathBuf) -> Result<Config, ConfigError> {
    ...
    // 3. Project-local (.c2/config.json)
    let project_path = working_dir.join(".c2").join("config.json");
...
fn system_config_path() -> PathBuf {
    ...
    {
        PathBuf::from("C:\\ProgramData\\c2\\config.json")
    }
    #[cfg(not(target_os = "windows"))]
    {
        PathBuf::from("/etc/c2/config.json")
    }
}
